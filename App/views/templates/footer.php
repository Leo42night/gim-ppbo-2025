<!-- Intial Data -->
<script src="<?= BASE_URL ?>/js/constant.js"></script>
<script src="<?= BASE_URL ?>/js/init.js"></script>

<!-- mostly used in all module -->
<script src="<?= BASE_URL ?>/js/flash-card.js"></script>
<script src="<?= BASE_URL ?>/js/sound.js"></script>

<script src="<?= BASE_URL ?>/js/helper.js"></script>

<!-- need only after sound initial & before popup -->
<script src="<?= BASE_URL ?>/js/quiz.js"></script>

<!-- utils: need by trigger ui -->
<script src="<?= BASE_URL ?>/js/popup.js"></script>
<script src="<?= BASE_URL ?>/js/gallery.js"></script>

<!-- Need by Helper, special case -->
<script src="<?= BASE_URL ?>/js/login.js"></script>
<script src="<?= BASE_URL ?>/js/rating.js"></script>
<script src="<?= BASE_URL ?>/js/rate.js"></script>

<!-- Main logic & event handler -->
<script src="<?= BASE_URL ?>/js/trigger-ui.js"></script>
<script src="<?= BASE_URL ?>/js/main.js"></script>

</body>
</html>